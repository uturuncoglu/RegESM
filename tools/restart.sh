...
if [ $((lno-1)) -eq "0" ]; then
  ...
  pr=`ncdump -h output/ocean_rst_$dstamp.nc | grep CPP_options | grep PERFECT_RESTART`
  if [ -z "$pr" ]; then
    ...
    ncks -d ocean_time,$((lno-3)),$((lno-1)) output/ocean_rst_$dstamp.nc output/ocean_rst.nc
  else
